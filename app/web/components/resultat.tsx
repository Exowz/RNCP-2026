  ...
  return <>
    ...
    <section className="result-heading" aria-labelledby="titre-resultat">
      <div><p className="eyebrow">Cas évalué</p><h1 id="titre-resultat">{presentation.type_local} de {presentation.surface_reelle_bati} m² à {presentation.nom_commune}</h1>
        <p>Vente du {new Intl.DateTimeFormat("fr-FR", { dateStyle: "long" }).format(new Date(presentation.date_mutation))}.{presentation.adresse_ban ? ` Adresse du DPE : ${presentation.adresse_ban}.` : " Aucun DPE n'a été rapproché."}</p>
      </div>
    ...
    <section aria-live="polite" aria-labelledby="verdict-heading" className="result-live">
      ...
      <div className="axis-grid">
        ...
        <article className="axis axis-anomalie"><h3>Ressemblance avec les autres cas</h3><p className="score">{verdict.niveau_anomalie.replaceAll("_", " ")}</p><p>{lectureAnomalie[verdict.niveau_anomalie]}</p></article>
        <article className="axis axis-confiance"><h3>Confiance dans cette lecture</h3><p className="score">{verdict.confiance.niveau}</p><p>{lectureConfiance[verdict.confiance.niveau]}</p></article>
      </div>
    </section>
    ...
    <section className="evidence-section"><h2>Pourquoi ce résultat ?</h2>{verdict.motifs.length ? <ul className="reason-list">{verdict.motifs.map((motif) => <li key={motif.identifiant}><strong>{motif.libelle}.</strong> {motif.message}</li>)}</ul> : <p>Aucune contradiction de cohérence n'a été déclenchée par les règles connues.</p>}<p>Exposition aux aléas : niveau {verdict.exposition_aleas.niveau_max} sur 4, avec {verdict.exposition_aleas.nb_aleas_significatifs} aléa(x) significatif(s).</p></section>
